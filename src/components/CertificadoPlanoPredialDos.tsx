...
import { generatePDF } from "@/services/pdfCertificadoPlanoPredialDos";
import { Certificado } from "@/types";

const CertificadoPlanoPredialDos: React.FC = () => {
    ...
    <div>
      ...
      {inscripcion && (
        <div className="flex flex-col gap-2 rounded-sm bg-sky-200 p-5 mt-10 w-fit">
          <h2 className="font-bold text-xl">Certificado Plano Predial Catastral -- DOS</h2>
          <p>
            <span className="font-semibold">Referencia Catastral No:</span>{" "}
            {inscripcion.predial}
          </p>
          ...
          >
            Generar Certificado Plano Predial Catastral
          </button>
        </div>
      )}
    </div>
...
export default CertificadoPlanoPredialDos;